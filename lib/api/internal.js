...
module.exports = function (couchdb){
    ...
    this.gameCodes = function (options, callback){
        if (typeof(options) == "function") { callback = options, options = {}; }

        var creq = new self.couchdb.Request("GET", "/_design/game/_view/codes/"+util.encodeOptions(options), callback);
    };

    this.gameNames = function (options, callback){
        if (typeof(options) == "function") { callback = options, options = {}; }

        var creq = new self.couchdb.Request("GET", "/_design/game/_view/names/"+util.encodeOptions(options), callback);
    };

    this.gameGenres = function (options, callback){
        if (typeof(options) == "function") { callback = options, options = {}; }
        if (typeof options.reduce == "undefined") options.reduce = false;

        var creq = new self.couchdb.Request("GET", "/_design/game/_view/genres/"+util.encodeOptions(options), callback);
    };

    this.gameGenreCount = function (options, callback){
        if (typeof(options) == "function") { callback = options, options = {}; }
        if (typeof options.group == "undefined") options.group = true;
