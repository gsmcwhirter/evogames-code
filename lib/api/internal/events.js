...
API.prototype.startdates = function (options, callback){
    if (typeof(options) == "function") { callback = options, options = {}; }

    var creq = new this.couchdb.Request("GET", "/_design/event/_view/startdates/"+util.encodeOptions(options), callback);
};

API.prototype.game_startdates = function (options, callback){
    if (typeof(options) == "function") { callback = options, options = {}; }

    var creq = new this.couchdb.Request("GET", "/_design/event/_view/game_startdates/"+util.encodeOptions(options), callback);
};
...
API.prototype.admin_startdates = function (options, callback){
    if (typeof(options) == "function") { callback = options, options = {}; }

    var creq = new this.couchdb.Request("GET", "/_design/event/_view/admin_startdates/"+util.encodeOptions(options), callback);
};
