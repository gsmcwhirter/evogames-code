...
function checkGroupAdmin(req, res, next){
    if (req.player && req.group){
        if (req.group.owners.indexOf(req.player._id) > -1){
...
function displayProfile(req, res, next){
    ...
    base.util.inParallel(
        ...
        function (){
            res.render("group/profile", {group: group, gdescription: description, owners: owners, crumbs: crumbs, editable: req.isGroupAdmin});
        }
    );
}
