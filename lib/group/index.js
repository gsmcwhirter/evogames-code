...
app.get("/:code/event-history", displayEventHistory);
...
app.get("/:code/controls/einvites", base.auth.loginCheck, mw.forceGroupAdmin("events"), eInviteControls);
app.get("/:code/controls/eregistrations", base.auth.loginCheck, mw.forceGroupAdmin("events"), eRegistrationControls);
        ...
        function afterUnique(){
            ...
            else
            {
                ...
                api.putDoc(group, function (response){
                    if (response.error)
                    {
                        req.flash('error','Unable to save your group: '+(response.reason || response.error));
                        createForm(req, res, next, locals);
                    }
                    else
                    {
            ...
            api.putDoc(group, function (response){
                if (response.error){
                    ...
                    if (group.join_type == "open" || is_invited){
                        ...
                        (group.members || []).filter(function (member){return member.approved && (member.admin || []).indexOf("members") > -1}).forEach(function (admin){
                            msg.cc.push({handle: admin.handle, is_read: false, is_deleted: false});
                        });

                        msg.subject = "New Member Joined: "+msg.subject;

                        pm_template = "mail/group_member_joined.ejs";
                    }
                    else {
                        req.flash('info', 'You have successfully applied to join '+group.name);

                        msg.subject = "New Membership Application: "+msg.subject;

                        (group.members || []).filter(function (member){return member.approved && (member.admin || []).indexOf("requests") > -1}).forEach(function (admin){
                            msg.cc.push({handle: admin.handle, is_read: false, is_deleted: false});
                        });

                        pm_template = "mail/group_member_applied.ejs";
                    }

                    base.util.render_pm(res, pm_template, {group: group, alias: fields.alias, handle: req.player.handle}, msg, function (err, message){
                        if (!err){
...
function leaveProcess(req, res, next){
    ...
    if (mship_found){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                res.send({ok: false, error: 'Unable to leave the group: '+(response.reason || response.error)});
            }
            else {
                res.send({ok: true, info: "Successfully left "+group.name+"."});
            }
        });
    }
    else {
        res.send({ok: false, error: 'Unable to find that group membership record.'});
    }
    ...
    if (invite_found){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                ...
                res.send({ok: true, info: "Successfully declined the invitation to "+group.name+"."});
            }
        });
    }
    else {
        res.send({ok: false, error: 'Unable to find that group invitation record.'});
    }
}
...
function displayProfile(req, res, next){
    ...
    var locals = {};
    var group = locals.group = req.group;

    locals.crumbs = [{href: "/", text: "Home"},
                    {href: "/group", text: "Groups"},
                    {href: "/group/"+group.code, text: group.code}];

    locals.gdescription = markdown.parse(group.description || "(no description)\n");
    locals.member_count = 0;
    (group.members || []).forEach(function (member){
        if (member.approved){
            locals.member_count++;
        }
    });
    locals.owners = group.owners || [];
    locals.editable = req.groupAdmin;
    locals.current_events = [];
    locals.future_events = [];

    base.util.inParallel(
        function (callback){
            api.events.registration_enddates({startkey: [req.group._id, 0, now], endkey: [req.group._id, 1], include_docs: true}, function (response){
                if (response.rows && response.rows.length){
        ...
        function (){
            res.render("group/profile", locals);
        }
    );
...
function displayEventHistory(req, res, next){
    var api = req.app.set('iapi');
    var locals = {};
    locals.crumbs = [{href: "/", text: "Home"},
                        {href: "/group", text: "Groups"},
                        {href: "/group/"+req.group.code, text: req.group.code},
                        {href: "/group/"+req.group.code+"/event-history", text: "Events"}];
    locals.group = req.group;
    locals.editable = req.groupAdmin;
    locals.past_events = [];

    api.events.registration_enddates({startkey: [req.group._id, 0], endkey: [req.group._id, 0, (new Date()).toISOString(), 1], include_docs: true}, function (response){
       if (response.rows && response.rows.length){
           locals.past_events = response.rows.map(function (row){return {event: row.value.event, game: row.doc};});
       }

       res.render("group/event_history", locals);
    });
}
    ...
    else
    {
        ...
        else
        {
            api.putDoc(group, function (response){
                ...
                {
                    locals.errors = true;
                    req.flash('error','Unable to save your group information: '+(response.reason || response.error));
                    editForm(req, res, next, locals);
                }
                else
                {
...
function permissionsProcess(req, res, next){
    ...
    if (!req.body){
        ...
        if (found_member_index > -1){
            ...
            api.putDoc(group, function (response){
                if (response.error){
                    req.flash('error', 'Unable to save the member privileges: '+(response.reason || response.error));
        ...
        api.players.handles({key: req.body.handle.toLowerCase()}, function (response){
            if (response.error || !response.rows || !response.rows.length){
                ...
                if (invite_exists){
                    ...
                    api.putDoc(group, function (response){
                        if (response.error){
                            res.send({ok: false, error: "Unable to save the invitation: "+(response.reason || response.error)});
                        }
                        else {
                            var msg = {
                                from: "",
                                subject: "Invitation to "+group.name+" ("+group.code+")",
                                to: [{handle: req.body.handle, is_read: false, is_deleted: false}],
                                bcc: [{handle: req.player.handle, is_read: false, is_deleted: false}]
                            };

                            base.util.render_pm(res, "mail/group_invite.ejs", {group: group, handle: req.body.handle}, msg, function (err, message){
                                if (message){
                                    api.messages.sendMessage(message);
                                }
                            });

                            res.send({ok: true, info: "Invitation successfully saved."});
                        }
                    });
                }
            }
    ...
    if (invite_index > -1){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                ...
                res.send({ok: true, info: "Successfully withdrew the invitation to "+req.group.name+"."});
            }
        });
    }
    else {
    ...
    if (member_index > -1){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                ...
                var msg = {
                    from: "",
                    subject: "Request to Join "+group.name+" ("+group.code+")",
                    to: [{handle: handle, is_read: false, is_deleted: false}],
                    bcc: [{handle: req.player.handle, is_read: false, is_deleted: false}]
                };

                base.util.render_pm(res, "mail/group_request_approve.ejs", {group: group, alias: alias, handle: handle}, msg, function (err, message){
                    if (!err){
                        api.messages.sendMessage(message);
                    }
                });

                res.send({ok: true, info: "Successfully approved the membership request from <span class='alias'>"+alias+"</span><span class='handle'>@"+handle+"</span>."});
            }
        });
    }
    else {
        res.send({ok: false, error: 'Unable to find that membership request.'});
    }
}
    ...
    if (mship_index > -1){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                ...
                base.util.render_pm(res, "mail/group_request_deny.ejs", {group: group, alias: alias, handle: handle}, msg, function (err, message){
                    if (!err){
                        api.messages.sendMessage(message);
                    }
                });

                res.send({ok: true, info: "Successfully denied the membership request from <span class='alias'>"+alias+"</span><span class='handle'>@"+handle+"</span>."});
            }
        });
    }
    else {
...
function removeMember(req, res, next){
    ...
    if (mship_index > -1){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                res.send({ok: false, error: 'Unable to remove the member: '+(response.reason || response.error)});
            }
            else {
                res.send({ok: true, info: "Successfully removed <span class='alias'>"+alias+"</span><span class='handle'>@"+handle+"</span> from the group."});
            }
        });
    }
    else {
        res.send({ok: false, error: 'Unable to find membership record.'});
    }
}
        ...
        api.players.handles({key: req.body.handle.toLowerCase()}, function (response){
            if (response.error || !response.rows || !response.rows.length){
                ...
                (req.group.owners || []).forEach(function (owner){
                    if (owner.toLowerCase() == response.rows[0].value.handle.toLowerCase()){
                        owner_exists = true;
                    }
                });

                if (owner_exists){
                    res.send({ok: false, error: 'That player is already a group owner.'});
                }
                else {
                    var group = base.util.clone(req.group);
                    var handle = response.rows[0].value.handle;
                    group.owners.push(handle);
                    
                    api.putDoc(group, function (response){
                        if (response.error){
                            res.send({ok: false, error: "Unable to add the player as an owner: "+(response.reason || response.error)});
                        }
                        else {
                            var msg = {
                                from: "",
                                subject: "Ownership of "+group.name+" ("+group.code+")",
                                to: [{handle: handle, is_read: false, is_deleted: false}],
                                bcc: [{handle: req.player.handle, is_read: false, is_deleted: false}]
                            };

                            base.util.render_pm(res, "mail/group_owner_added.ejs", {group: group, handle: handle}, msg, function (err, message){
                                if (!err){
                                    api.messages.sendMessage(message);
                                }
                            });

                            res.send({ok: true, info: "Player successfully added as an owner."});
                        }
                    });
                }
            }
...
function resignOwnership(req, res, next){
    ...
    if (owner_index > -1){
        ...
        if (group.owners.length < 1){
            res.send({ok: false, error: 'You cannot resign group ownership if you are the last owner.'});
        }
        else {
            api.putDoc(group, function (response){
                if (response.error){
                    res.send({ok: false, error: 'Unable to resign ownership: '+(response.reason || response.error)});
                }
                else {
                    res.send({ok: true, info: "Successfully resigned ownership of "+req.group.name+"."});
                }
            });
        }
    }
    else {
        res.send({ok: false, error: 'You are not a group owner.'});
    }
}
...
function eInviteControls(req, res, next){
    ...
    var locals = {
        ...
        crumbs: [{href: "/", text: "Home"},
                    ...
                    {href: "/group/"+req.group.code, text: req.group.code},
                    {href: "/group/"+req.group.code+"/controls", text: "Manage"},
                    {href: "/group/"+encodeURI(req.group.code)+"/controls/einvites", text: "Event Invitations"}],
        invitations: []
    };

    base.util.inParallel(
        function (callback){
            api.events.invitations({startkey: [req.group._id], endkey: [req.group._id, 1], include_docs: true}, function (response){
                if (response.rows && response.rows.length){
                    locals.invitations = response.rows.map(function (row){return {event: row.value.event, game: row.doc};}).filter(function (invite){return invite.event.event_type == "group_register";});
                }
        ...
        function (){
            res.render("group/einvites", locals);
        }
    );
}
...
function eRegistrationControls(req, res, next){
    ...
    var locals = {
        ...
        crumbs: [{href: "/", text: "Home"},
                    ...
                    {href: "/group/"+req.group.code, text: req.group.code},
                    {href: "/group/"+req.group.code+"/controls", text: "Manage"},
                    {href: "/group/"+encodeURI(req.group.code)+"/controls/eregistrations", text: "Event Registrations"}],
    ...
    base.util.inParallel(
        ...
        function (){
            res.render("group/eregistrations", locals);
        }
    );
}