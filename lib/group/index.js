...
function leaveProcess(req, res, next){
    ...
    if (mship_found){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                res.end(JSON.stringify({ok: false, error: 'Unable to leave the group: '+(response.reason || response.error)}));
            }
            else {
...
function declineProcess(req, res, next){
    ...
    if (invite_found){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                res.end(JSON.stringify({ok: false, error: 'Unable to decline the invitation: '+(response.reason || response.error)}));
            }
            else {
        ...
        api.players.handles({key: req.body.handle.toLowerCase()}, function (response){
            if (response.error || !response.rows || !response.rows.length){
                ...
                if (invite_exists){
                    ...
                    api.putDoc(group, function (response){
                        if (response.error){
                            res.end(JSON.stringify({ok: false, error: "Unable to save the invitation: "+(response.reason || response.error)}));
                        }
                        else {
...
function withdrawInvite(req, res, next){
    ...
    if (invite_index > -1){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                res.end(JSON.stringify({ok: false, error: 'Unable to withdraw the invitation: '+(response.reason || response.error)}));
            }
            else {
...
function approvePending(req, res, next){
    ...
    if (member_index > -1){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                res.end(JSON.stringify({ok: false, error: 'Unable to approve the request: '+(response.reason || response.error)}));
            }
            else {
...
function denyPending(req, res, next){
    ...
    if (mship_index > -1){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                res.end(JSON.stringify({ok: false, error: 'Unable to deny the request: '+(response.reason || response.error)}));
            }
            else {
...
function removeMember(req, res, next){
    ...
    if (mship_index > -1){
        ...
        api.putDoc(group, function (response){
            if (response.error){
                res.end(JSON.stringify({ok: false, error: 'Unable to remove the member: '+(response.reason || response.error)}));
            }
            else {
        ...
        api.players.handles({key: req.body.handle.toLowerCase()}, function (response){
            if (response.error || !response.rows || !response.rows.length){
                ...
                if (owner_exists){
                    ...
                    api.putDoc(group, function (response){
                        if (response.error){
                            res.end(JSON.stringify({ok: false, error: "Unable to add the player as an owner: "+(response.reason || response.error)}));
                        }
                        else {
...
function resignOwnership(req, res, next){
    ...
    if (owner_index > -1){
        ...
        if (group.owners.length < 1){
            ...
            api.putDoc(group, function (response){
                if (response.error){
                    res.end(JSON.stringify({ok: false, error: 'Unable to resign ownership: '+(response.reason || response.error)}));
                }
                else {
