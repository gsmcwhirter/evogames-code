...
app.get("/directory", showDirectory);
...
app.get("/:code/controls", base.auth.loginCheck, checkGroupAdmin, forceGroupAdmin(), groupControls);

app.get("/:code/controls/edit", base.auth.loginCheck, checkGroupAdmin, forceGroupAdmin("edit"), editForm);
app.post("/:code/controls/edit", base.auth.loginCheck, checkGroupAdmin, forceGroupAdmin("edit"), editProcess);

app.get("/:code/controls/owners", base.auth.loginCheck, checkGroupAdmin, forceGroupOwner, ownersForm);
app.put("/:code/controls/owners/add", base.auth.loginCheck, checkGroupAdmin, forceGroupOwner, addOwner);
app.del("/:code/controls/owners/resign", base.auth.loginCheck, checkGroupAdmin, forceGroupOwner, resignOwnership);

app.get("/:code/controls/requests", base.auth.loginCheck, checkGroupAdmin, forceGroupAdmin("requests"), pendingForm);
app.del("/:code/controls/requests/:alias@:handle/approve", base.auth.loginCheck, checkGroupAdmin, forceGroupAdminAjax("requests"), approvePending);
app.del("/:code/controls/requests/:alias@:handle/deny", base.auth.loginCheck, checkGroupAdmin, forceGroupAdminAjax("requests"), denyPending);

app.get("/:code/controls/members", base.auth.loginCheck, checkGroupAdmin, forceGroupAdmin("members"), membersForm);
app.get("/:code/controls/members/:alias@:handle", base.auth.loginCheck, checkGroupAdmin, forceGroupAdmin("members"), permissionsForm);
app.post("/:code/controls/members/:alias@:handle", base.auth.loginCheck, checkGroupAdmin, forceGroupAdmin("members"), permissionsProcess);
app.del("/:code/controls/members/:alias@:handle/remove", base.auth.loginCheck, checkGroupAdmin, forceGroupAdminAjax("members"), removeMember);

app.get("/:code/controls/invites", base.auth.loginCheck, checkGroupAdmin, forceGroupAdmin("invites"), inviteForm);
app.put("/:code/controls/invites/add", base.auth.loginCheck, checkGroupAdmin, forceGroupAdminAjax("invites"), addInvite);
app.del("/:code/controls/invites/@:handle", base.auth.loginCheck, checkGroupAdmin, forceGroupAdminAjax("invites"), withdrawInvite);
    ...
    else
    {
        ...
        api.groupCodes(function (response){
            if (response.rows && response.rows.length){
                if (response.rows.map(function (item){return item[0];}).indexOf(group.code.toLowerCase()) > -1)
                {
                    locals.errors = true;
...
function displaySearch(req, res, next){
    ...
    if (req.param("query")){
        ...
        if (locals.query.length < 3){
            ...
            api.groupSearch({startkey: locals.query, endkey: locals.query}, function (response){
                if (response.error){
                    ...
                    (response.rows || []).forEach(function (row){
                        stringrep = JSON.stringify([row.value.code, row.value.name, row.value.source]);
...
function showDirectory(req, res, next){
    ...
    var locals = {
        ...
        crumbs: [{href: "/", text: "Home"},
                 {href: "/group", text: "Groups"},
                 {href: "/group/directory", text: "Directory"}]
    };

    api.groupNames({include_docs: true}, function (response){
        if (response.error){
            req.flash('error', 'Unable to retrieve a list of groups.');
        }
        else {
            var grps = us._(response.rows).pluck('doc');
            var tmpgrp = [];
            var curletter = false;
            var curindex = -1;
            var nextletter;
        ...
        res.render("group/directory", locals);
    });
}
...
function ownersForm(req, res, next){
    ...
    base.util.inParallel(
        function (callback){
            api.ownersByGroup({key: group.code, include_docs: true}, function (response){
                if (response.rows){
                    locals.owners = us._(response.rows).pluck('doc');
                }
        ...
        api.userList({startkey: [req.body.handle.toLowerCase()], endkey: [req.body.handle.toLowerCase(), req.body.handle.toUpperCase()]}, function (response){
            if (response.error || !response.rows || !response.rows.length){
                ...
                if (owner_exists){
                    ...
                    req.group.owners.push(response.rows[0].id);
                    api.putDoc(req.group, function (response){
                        if (response.error){
...
function resignOwnership(req, res, next){
    ...
    (req.group.owners || []).forEach(function (owner, index){
        if (owner == req.player._id){
            owner_index = index;
        }
    });

    if (owner_index > -1){
        req.group.owners.splice(owner_index, 1);

        if (req.group.owners.length < 1){
