	...
	function checkProp(value, schema, path,i){
		...
		if(value === undefined){
			...
			if(value !== null){
				...
				{ 
				    if (typeof(schema.pattern) == "object")
				    {
				        if (schema.pattern.negative)
				        {
				            if (value.match(schema.pattern.pattern))
				            {
				                addError(schema.pattern.message || "has invalid format.");
				            }
				        }
				        else
				        {
