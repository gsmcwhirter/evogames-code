    ...
    api.getDoc(message_id, function (response){
        if (response && response.type == "message"){
            if (response.from.toLowerCase() == req.player.handle.toLowerCase()){
                ...
                var tos = (response.to || []).filter(function (rec){return !rec.is_deleted;}).map(function (rec){return rec.handle.toLowerCase()});
                var ccs = (response.cc || []).filter(function (rec){return !rec.is_deleted;}).map(function (rec){return rec.handle.toLowerCase()});
                var bccs = (response.bcc || []).filter(function (rec){return !rec.is_deleted;}).map(function (rec){return rec.handle.toLowerCase()});
