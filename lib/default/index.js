    ...
    base.util.inParallel(
        function (callback){
            api.news.published({include_docs: true, descending: true, limit: 10}, function (response){
                if (response.rows)
                {
                    articles = response.rows.map(function (row){return row.doc;});
                }
        ...
        function (callback){
            api.players.recent({include_docs: true, descending: true, limit: 10}, function (response){
                if (response.rows)
                {
                    registrations = response.rows.map(function (row){return row.doc;});
                }
        ...
        function (callback){
            api.groups.recent({include_docs: true, descending: true, limit: 10}, function (response){
                if (response.rows){
                    groups = response.rows.map(function (row){return row.doc;});
                }
