...
function slugParser(req, res, next, slug){
    ...
    api.events.slugs({key: slug.toLowerCase(), include_docs: true}, function (response){
        if (response.rows && response.rows.length){
            if (response.rows[0].doc.gameid == req.game._id){
...
function matchIdParser(req, res, next, matchid){
    var api = req.app.set('iapi');
    if (req.event){
        api.getDoc("event-match-"+matchid, function (response){
            if (!response.error && response.type == "event-match" && response.eventid == req.event._id){
                req.match = response;
                next();
            }
            else {
