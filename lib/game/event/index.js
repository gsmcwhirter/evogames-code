    ...
    if (req.body && req.body.teams){
        ...
        for (var tkey in req.body.teams){
            ...
            for (var pkey in req.body.teams[tkey].players){
                ...
                player.stats = req.body.teams[tkey].players[pkey].stats;

                for (var skey in player.stats){
                    if (typeof gametype.statdefs[skey.toLowerCase()] != "undefined"){
                        if (gametype.statdefs[skey.toLowerCase()].valtype == "integer"){
                            player.stats[skey] = parseInt(player.stats[skey]);
                        }
                        else if (gametype.statdefs[skey.toLowerCase()].valtype == "float"){
                            player.stats[skey] = parseFloat(player.stats[skey]);
                        }
                        else if (gametype.statdefs[skey.toLowerCase()].valtype == "enum"){
                            if (gametype.statdefs[skey.toLowerCase()].enum_list.indexOf(player.stats[skey]) == -1){
                                player.stats[skey] = false;
                            }
                        }
                        else {
                            player.stats[skey] = (player.stats[skey] || "").trim();
                        }
                    }
                }
            ...
            for (var i = 0, cti = match.teams.length; i < cti; i++){
                ...
                for (var j = 0, ctj = match.teams[i].players.length; j < ctj; j++){
                    ...
                    gametype.stats.forEach(function (stat){
                        ...
                        if (typeof match.teams[i].players[j].stats[stat.name] == "undefined"){
                        ...
                        if (stat.valtype == "formula"){
                            match.teams[i].players[j].stats[stat.name] = gametype.statdefs[stat.name.toLowerCase()].statfunc(match.teams[i].players[j].stats);
                        }

                        //verify enums are from the list
                        if (stat.valtype == "enum"){
                            if (!match.teams[i].players[j].stats[stat.name]){
...
function displayMatch(req, res, next){
    ...
    locals.crumbs = [{href: "/", text: "Home"},
                        ...
                        {href: "/game/"+req.game.code+"/event/"+req.event.slug, text: req.event.name},
                        {href: "/game/"+req.game.code+"/event/"+req.event.slug+"/matches", text: "Matches"},
                        {href: "/game/"+req.game.code+"/event/"+req.event.slug+"/matches/"+req.match._id.substring(12), text: "View"}];
