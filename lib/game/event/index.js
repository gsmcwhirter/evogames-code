...
app.get(_base+"/:slug/matches/:matchid/dispute", base.auth.loginCheck, mw.forceDisputable, disputeForm);
app.post(_base+"/:slug/matches/:matchid/dispute", base.auth.loginCheck, mw.forceDisputable, disputeProcess);
...
function cancelRegistration(req, res, next){
    ...
    if (req.regis_index > -1){
        ...
        if (event.registrations[req.regis_index].approved && event.registrations[req.regis_index].type == req.regis_data.type && event.registrations[req.regis_index].id == req.regis_data.id){
            ...
            api.putDoc(event, function (response){
                if (response.error){
                    res.end(JSON.stringify({ok: false, error: 'Unable to cancel the registration: '+(response.reason || response.error)}));
                }
                else {
...
function removeAdmin(req, res, next){
    ...
    if (req.admin_index > -1 && (req.event.admins || [])[req.admin_index].toLowerCase() == req.admin_handle.toLowerCase()){
        ...
        api.putDoc(event, function (response){
            if (response.error){
                res.end(JSON.stringify({ok: false, error: 'Unable to remove the admin: '+(response.reason || response.error)}));
            }
            else {
...
function deleteEvent(req, res, next){
    ...
    if (req.event.startdate && req.event.startdate <= (new Date()).toISOString()){
        ...
        api.delDoc(req.event, function (response){
            if (response.error){
                res.end(JSON.stringify({ok: false, error: "cancellation failed: "+(response.reason || response.error)}));
            }
            else {
                ...
                api.uuids(function (uuids){
                    ...
                    api.putDoc(match, function (response){
                        if (!response.error){
                            ...
                            res.writeHead(200, {"Content-type": "application/json"});
                            res.end(JSON.stringify({ok: false, error: "There was an error submitting the match: "+(response.reason || response.error)}));
                        }
                    });
                });
            }
...
function editMatchProcess(req, res, next){
    ...
    if (req.body && typeof req.body.teams != "undefined" && typeof req.body.disputeid != "undefined" && typeof req.body.resolution_note != "undefined"){
        ...
        if (dispute_index > -1){
                ...
                if (errors.length == 0){
                    ...
                    api.putDoc(match, function (response){
                        if (!response.error){
                            ...
                            res.writeHead(200, {"Content-type": "application/json"});
                            res.end(JSON.stringify({ok: true, info: "", redir_url: "/game/"+req.game.code+"/event/"+req.event.slug+"/controls/disputes"}));
                        }
                        else {
                            res.writeHead(200, {"Content-type": "application/json"});
                            res.end(JSON.stringify({ok: false, error: "There was an error resolving the dispute: "+(response.reason || response.error)}));
                        }
                    });
                }
                else {
            ...
            res.writeHead(200, {"Content-type": "application/json"});
            res.end(JSON.stringify({ok: false, error: "The pending dispute you tried to edit was not found or was already resolved."}));
        }
    }
    else {
        res.writeHead(200, {"Content-type": "application/json"});
...
function _toggleDisputes(allow){
    return function (req, res, next){
        ...
        api.putDoc(event, function (response){
            if (!response.error){
                res.end(JSON.stringify({ok: true, info: "The event dispute policy was successfully changed."}));
            }
            else {
                res.end(JSON.stringify({ok: false, error: "The event dispute policy was unable to be changed: "+(response.reason || response.error)}));
            }
        });
    }
}
...
function deleteMatch(req, res, next){
    ...
    api.delDoc(req.match, function (response){
        if (!response.error){
            ...
            res.writeHead(200, {"Content-type": "application/json"});
            res.end(JSON.stringify({ok: false, error: "There was an error deleting the disputed match: "+(response.reason || response.error)}));
        }
    });
}
    ...
    if (req.body && typeof req.body.disputeid != "undefined" && typeof req.body.resolution_note != "undefined"){
        ...
        if (dispute_index > -1){
            ...
            if (validation.valid){
                ...
                res.writeHead(200, {"Content-type": "application/json"});
                res.end(JSON.stringify({ok: false, error: "Dispute format validation failed."}));
            }
        }
        else {
            res.writeHead(200, {"Content-type": "application/json"});
            ...
            api.events.event_stats({startkey: [req.event._id, 0], endkey: [req.event._id, 1]}, function (response){
                if (response.rows && response.rows.length){
                    locals.players = response.rows.map(function (row){
                        locals.relevant_stats.forEach(function (stat, key){
                            if (stat.valtype == "function"){
                                row.value[key] = stat.statfunc(row.value);
                            }
                        });
                ...
                if (locals.players.length){
                    ...
                    for (var i = 1, cti = locals.players.length; i < cti; i++){
                        ...
                        if (_sortfunc(locals.players[i], locals.players[i-1]) == 0){
                            ...
                            locals.players[i]["0rank"] = locals.players[i-1]["0rank"] + 1;
                        }
                    }
                }
