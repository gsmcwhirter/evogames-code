            ...
            if (validation.valid){
                ...
                api.putDoc(match, function (response){
                    if (!response.error){
                        ...
                        req.flash("error", "There were errors recording your dispute: "+response.error);
                ...
                if (errors.length == 0){
                    ...
                    api.putDoc(match, function (response){
                        if (!response.error){
                            ...
                            res.writeHead(200, {"Content-type": "application/json"});
                            res.end(JSON.stringify({ok: false, error: "There was an error resolving the dispute: "+response.error}));
                        }
                    });
                }
                else {
    ...
    return function (req, res, next){
        ...
        api.putDoc(event, function (response){
            if (!response.error){
                ...
                res.end(JSON.stringify({ok: false, error: "The event dispute policy was unable to be changed: "+response.error}));
            }
        });
    }
}
...
function deleteMatch(req, res, next){
    ...
    api.delDoc(req.match, function (response){
        if (!response.error){
            ...
            res.writeHead(200, {"Content-type": "application/json"});
            res.end(JSON.stringify({ok: false, error: "There was an error deleting the disputed match: "+response.error}));
        }
    });
}
