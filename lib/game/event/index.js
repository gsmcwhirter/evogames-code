    ...
    if (req.event.event_type == "player_register"){
        (req.event.registrations || []).forEach(function (regis){
            if (regis.type == "player" && regis.approved){
                ...
                if (!locals.autochange_data.sel1[regis.code_or_handle]){
                    locals.autochange_data.sel1[regis.code_or_handle] = [];
                }

                locals.autochange_data.sel1[regis.code_or_handle].push(regis.name_or_alias);

                if (!locals.autochange_data.sel2[regis.name_or_alias]){
                    locals.autochange_data.sel2[regis.name_or_alias] = [];
                }

                locals.autochange_data.sel2[regis.name_or_alias].push(regis.code_or_handle);
            }
        });
        ...
        api.events.registrations({startkey: [1, req.event._id, 0, 1], include_docs: true}, function (response){
            if (response.rows && response.rows.length){
                response.rows.forEach(function (row){
