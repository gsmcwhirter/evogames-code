...
function eventDirectory(req, res, next){
    ...
    base.util.inParallel(
        function (callback){
            api.events.game_enddates({startkey: [req.game._id, 0], endkey: [req.game._id, -1 * (new Date()).getTime(), 0, (new Date()).toISOString()]}, function (response){
                ...
                api.uuids(function (uuids){
                    ...
                    api.putDoc(event, function (response){
                        if (!response.error){
                            ...
                            res.redirect(req.game.code+"/event/"+event.slug);
                        }
                        else {
