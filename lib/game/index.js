...
app.get("/:code/gametypes", base.auth.permissionCheck("game admin", true), gametypeList);
app.get("/:code/gametypes/edit", base.auth.loginCheck, base.auth.permissionCheck("game admin"), gametypeForm);
...
function gametypeList(req, res, next){
    ...
    for (var i = 0, cti = (req.game.gametypes || []).length; i < cti; i++){
        req.game.gametypes[i].ratingformula_latex = mlexer.parseStringRepLatex(req.game.gametypes[i].ratingformula);

        for (var j = 0, ctj = (req.game.gametypes[i].stats || []).length; j < ctj; j++){
            if (req.game.gametypes[i].stats[j].valtype == "formula"){
                req.game.gametypes[i].stats[j].latex = mlexer.parseStringRepLatex(req.game.gametypes[i].stats[j].valformula);
            }
        }
    }
...
function gametypeForm(req, res, next){
    var locals = {
        ...
        crumbs: [
            ...
            {href: "/game/"+req.game.code, text: req.game.name},
            {href: "/game/"+req.game.code+"/gametypes", text: "Game Types"},
            {href: "/game/"+req.game.code+"/gametypes/edit", text: "Edit"}]
    };

    res.render("game/gametypes_edit", locals);
}
    ...
    if (!req.body){
        ...
        if (errors.length){
            ...
            api.putDoc(req.game, function (response){
                if (response.error){
                    res.send({ok: false, error: "Unable to save game type: "+(response.reason || response.error)});
                }
                else {
                    res.send({ok: true, info: "Game type saved successfully."});
                }
            });
        }
    }
...
function removeGametype(req, res, next){
    ...
    api.putDoc(game, function (response){
        if (response.error){
            res.send({ok: false, error: "Unable to delete game type: "+(response.reason || response.error)});
        }
        else {
            res.send({ok: true, info: "Game type deleted successfully."});
        }
    });
}
