...
app.put("/:code/gametypes/add", base.auth.loginCheckAjax, base.auth.permissionCheckAjax("game admin"), addGametype);
app.put("/:code/gametypes/:gtname", base.auth.loginCheckAjax, base.auth.permissionCheckAjax("game admin"), editGametype);
...
function parseGametype(req, res, next, gtname){
    if (!req.game){
        ...
        var gtfound = false;
        req.game.gametypes.forEach(function (gametype){
        ...
        if (!gtfound){
            res.writeHead(200, {"Content-type": "application/json"});
            res.end(JSON.stringify({error: "game type not found"}));
        }
        else {
            next();
        }
...
function addGametype(req, res, next){

}

function editGametype(req, res, next){
