...
function viewQuestion(req, res, next){
    ...
    if (req.question.status.answered || (req.player && (req.player.is_sysop || (req.player.permissions || []).indexOf("answer help") > -1))){
        ...
        res.render("help/question_view", locals);
    }
}
...
function editProcess(req, res, next){
    ...
    if (req.body)
    {
        ...
        question.tags = fields.tags.split(",").map(function (tag){ return tag.trim();}).filter(function (tag){return tag;});
...
function deleteQuestion(req, res, next){
    ...
    api.delDoc(req.question, function (response){
        if (!response.error){
            ...
            res.send({ok: true, info: "The question was successfully deleted."});
        }
        else {
            res.header("Content-type", "application/json");
            res.send({ok: false, error: "There was an error deleting the question: "+(response.reason || response.error)});
        }
    });
}