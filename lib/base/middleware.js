...
var forceSSL = function (req, res, next){
    ...
    if (port == "7080")
    {
        loc = "http://" + host + (':7443') + req.url;
    }
    else
    {
        loc = "https://" + host + (port ? ':'+port : '') + req.url;
    }
...
var forceNonSSL = function (){
    return function (req, res, next){
        ...
        if (port == "7443")
        {
            loc = "http://" + host + (':7080') + req.url;
        }
        else
        {
            loc = "http://" + host + (port ? ':'+port : '') + req.url;
        }
