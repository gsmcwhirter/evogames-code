...
var dynamicHelpers = {
    ...
    system: function (req, res){
        return req.app.set('config');
    },
    avatar: function (req, res){
        var sysconf = req.app.set('config');
        return function (email, size){
            if (typeof email == "object" && email.email_history && email.email_history.length)
            {
                if (email.gravatar_url)
                {
                    return email.gravatar_url + "&d=" + encodeURI("https://www.evogames.org" + sysconf.default_avatar);
                }
                else
                {
            ...
            email = email + "";
            size = size || 64;
            return util.gravatar_url(email, size) + "&d=" + encodeURI("https://www.evogames.org" + sysconf.default_avatar);
        };
    },
...
var datetimeHelpers = {
    date: dynamicHelpers.date,
    time: dynamicHelpers.time,
    datetime: dynamicHelpers.datetime
}
...
var handleError = function (err, req, res, next){
    switch (err.errcode){
        case 1: //NotFound
            res.render('errors/page_not_found', {layout: 'layout/error', _error: err}, function (err, str){
                if (err) str = err.toString();
                res.send(str, 404);
            });
            break;
        case 2: //AccessDenied
            res.render('errors/access_denied', {crumbs: [], _error: err});
            break;
        case 3: //LoggedIn
            res.render('errors/logged_in', {crumbs: [], _error: err, redirect: {href: '/'}});
            break;
        case 4: //NotLoggedIn
            res.render('errors/not_logged_in', {crumbs: [], _error: err, redirect: {href: '/login'}});
            break;
        case 5: //Forbidden
            res.render('errors/forbidden', {layout: 'layout/error', _error: err}, function (err, str){
                if (err) str = err.toString();
                res.send(str, 403);
            });
            break;
        case 6: //Lockout
            res.render('errors/lockout', {layout: 'layout/error', _error: err});
            break;
        case 7: //SysError
        default:
            res.render('errors/app_error', {layout: 'layout/error', _error: err}, function (err, str){
                if (err) str = err.toString();
                res.send(str, 500);
            });
    }
