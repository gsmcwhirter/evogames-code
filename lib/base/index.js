...
var setCookie = function (res, name, value, expires, path, domain, httponly)
{
    var cstr = name + "=" + value;
    ...
    res.header('Set-Cookie', cstr);
};

var clearCookie = function (res, name)
{
    return setCookie(res, name, '', dt.date("r", dt.strtotime("-7 days")));
}

var redirect = function (res, loc, status)
{
    res.writeHead(status, {"Location": loc, "Content-type": "text/html"});
    body = '<html><head><title>Page Moved</title></head><body><p>';
    body = body + 'The page you requested has permanently moved: <a href="'+loc+'">'+loc+'</a>.';
