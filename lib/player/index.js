...
function eventControls(req, res, next){
    ...
    base.util.inParallel(
        function (callback){
            api.events.admin_enddates({startkey: [req.player.handle.toLowerCase(), -10000000000000], endkey: [req.player.handle.toLowerCase(), -1 * (new Date()).getTime(), 0, (new Date()).toISOString()], include_docs: true}, function (response){
               if (response.rows && response.rows.length){
                   locals.current_events = response.rows.map(function (row){return {event: row.value.event, game: row.doc};});
               }
        ...
        function (callback){
            api.events.admin_startdates({endkey: [req.player.handle.toLowerCase(), -1 * (new Date()).getTime()], include_docs: true}, function (response){
                if (response.rows && response.rows.length){
                    locals.future_events = response.rows.map(function (row){return {event: row.value.event, game: row.doc};});
                }
