...
module.exports.urls = function (ssl, _base){
    ...
    {
        return function (app){
            app.get(_base + "/login", mw.forceSSL);
            app.get(_base + "/register", mw.forceSSL);
            app.get(_base + "/logout", mw.forceSSL);
            app.get(_base + "/confirm_email", mw.forceSSL);
            app.get(_base + "/lostpass", mw.forceSSL);
            app.get(_base + "/change_pass", mw.forceSSL);
            app.get(_base + "/change_email", mw.forceSSL);
...
function controlPanel(req, res, next){
    ...
    else
    {
        ...
        res.render("player/controls", {locals: locals});
    }
}
...
function aliasForm(req, res, next){
    ...
    else
    {
        ...
        res.render("player/aliases", {locals: locals});
    }
}
...
function datetimeForm(req, res, next, locals){
    ...
    else
    {
        ...
        res.render('player/datetime', {locals: locals});
    }
}
...
function groupControls(req, res, next){
    ...
    else
    {
        ...
        function whenDone(){
            if (done >= todo)
            {
                res.render('player/groups', {locals: locals});
            }
        }
    ...
    if (!req.params.handle && !req.player){
        ...
        api.userList({include_docs: true, startkey: [handle.toLowerCase()], endkey: [handle.toLowerCase(), handle.toUpperCase()]}, function (response){
            if (!response.errors && response.rows.length){
                ...
                res.render("player/profile", {locals: {player: player, crumbs: crumbs}});
            }
