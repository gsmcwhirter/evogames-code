...
app.param('handle', function (req, res, next, handle){
    ...
    api.userList({include_docs: true, key: handle.toLowerCase()}, function (response){
...
function registerProcess(req, res, next)
{
    ...
    else
    {
        ...
        api.userList(function (response){
            if (response.rows)
            {
                if (us._(response.rows).map(function (item){return item.key}).indexOf(player.handle.toLowerCase()) != -1)
                {
                    locals.errors = true;
            ...
            api.emailList(function (response){
                if (response.rows)
                {
                    if (us._(response.rows).map(function (item){return item.key.toLowerCase();}).indexOf(fields.email.toLowerCase()) != -1)
                    {
                        locals.errors = true;
        ...
        if (hplayer){
            ...
            if (req.params.alias || !req.hplayer){
                ...
                if (hplayer.aliases.indexOf(alias) < 0){
                    ...
                    base.util.inParallel(
                        function (callback){
                            api.groupsByOwner({startkey: [hplayer._id], endkey: [hplayer._id, 1], include_docs: true}, function (response){
                                if (response.rows){
                        ...
                        function (callback){
                            api.groupMembershipsById({startkey: [hplayer._id], endkey: [hplayer._id, 1], include_docs: true}, function (response){
                                if (response.rows){
                                    response.rows.forEach(function (row){
...
function groupControls(req, res, next){
    ...
    base.util.inParallel(
        function (callback){
            api.groupsByOwner({startkey: [req.player._id], endkey: [req.player._id, 1], include_docs: true}, function (response){
               if (response.rows){
        ...
        function (callback){
            api.groupAdmins({key: req.player._id, include_docs: true}, function (response){
                if (response.rows){
                    response.rows.forEach(function (row){
...
function displaySearch(req, res, next){
    ...
    if (req.param("query")){
        ...
        if (locals.query.length < 3){
            ...
            api.playerSearch({startkey: locals.query, endkey: locals.query}, function (response){
                if (response.error){
