        ...
        if (hplayer){
            ...
            if (req.params.alias || !req.hplayer){
                ...
                if (hplayer.aliases.indexOf(alias) < 0){
                    ...
                    base.util.inParallel(
                        function (callback){
                            api.groups.owners({startkey: [hplayer._id], endkey: [hplayer._id, 1], include_docs: true}, function (response){
                                if (response.rows){
...
function groupControls(req, res, next){
    ...
    base.util.inParallel(
        function (callback){
            api.groups.owners({startkey: [req.player._id], endkey: [req.player._id, 1], include_docs: true}, function (response){
               if (response.rows){
