...
app.get("", ticketIndex);
app.get("/", ticketIndex);
app.get("/:ticket", viewTicket);
app.get("/type/:type", viewType);
app.get("/status/:status", viewStatus);
app.get("/tag/:tag", viewTag);
app.get("/milestone/:milestone", viewMilestone);

app.put("/add", addTicket);
app.put("/:ticket/update", updateTicket);
app.put("/milestone/add", addMilestone);
...
function ticketIndex(req, res, next){
    ...
    api.ticketList({include_docs: true}, function (response){
        ...
        res.render('issues/list', {tickets: tickets, list_name: "Open Issues", crumbs: crumbs});
    });
}
